...
const useSearchFilters = ({ isSavedToUrl = true, queryParameterName = QueryParameterName.SEARCH }: Parameters = {}): {
  onSearch: (search: string) => void;
  search: string;
} => {
  const [searchParameters, setSearchParameters] = useSearchParams();
  const searchParameter = isSavedToUrl ? (searchParameters.get(queryParameterName) ?? '') : '';
  ...
  }, [search, searchParameters, queryParameterName, setSearchParameters, isSavedToUrl, isInitialLoad]);
