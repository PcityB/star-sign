...
import { calculateTotalPages, getInitialPage, getInitialPageSize } from './libs/helpers/helpers.js';
import { QueryParameterName } from '~/common/enums/app/app.js';
import { useCallback, useEffect, useState } from 'react';

type UsePagination = (parameters: { queryParameterPrefix: string; totalItemsCount: number }) => {
...
const usePagination: UsePagination = ({ queryParameterPrefix, totalItemsCount }) => {
  const [searchParameters, setSearchParameters] = useSearchParams();

  const pageParameterName = queryParameterPrefix + QueryParameterName.PAGE;
  const pageSizeParameterName = queryParameterPrefix + QueryParameterName.PAGE_SIZE;
  ...
  const [pageSize, setPageSize] = useState<number>(() => getInitialPageSize(NamedPageSizeParameter));
  ...
  }, [page, pageSize, setSearchParameters, pageParameterName, pageSizeParameterName]);
