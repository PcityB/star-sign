...
function isActionWithError(action: UnknownAction): action is UnknownAction & { error: SerializedError } {
  return 'error' in action && action.error !== undefined;
}

const handleSignOut = async (action: UnknownAction, listenerApi: ListenerEffectAPI<RootState, AppDispatch>) => {
