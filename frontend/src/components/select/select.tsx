...
const Select = <TFieldValues extends FieldValues, TOptionValue>({
  ...
  const labelClassName = getValidClassNames(styles['label-text'], isLabelHidden && 'visually-hidden');

  const value = useMemo(() => {
    const fieldValue = Array.isArray(field.value) ? field.value : [field.value];

    const matchedOptions = fieldValue.map((value) => options.find((option) => option.value === value)).filter(Boolean);

    return isMulti ? matchedOptions : (matchedOptions[FIRST_OPTION_INDEX] ?? null);
  ...
  const handleChange = useCallback(
    (
      ...
      if (isMulti) {
        const multiValues = (selectedOptions as MultiValue<SelectOption<TOptionValue>>).map(({ value }) => value);
        field.onChange(multiValues);
      } else {
        const singleValue = (selectedOptions as SingleValue<SelectOption<TOptionValue>>)?.value;
    ...
    <label className={styles['label']}>
      ...
      <ReactSelect
        classNames={{
          ...
          menu: () => getValidClassNames(styles['menu'], isBackgroundPrimary && styles['menu-primary']),
          ...
          singleValue: () => getValidClassNames(styles['single-value'], isSmall && styles['single-value-small']),
